...
var User = function () {
  ...
  _createClass(User, [{
    ...
    value: function getUserTypeByProduct(cookieMap, productType, ssoKey$$1) {
      var userType = userTypes$1.anonymous;
      if (cookieMap && cookieMap[ssoKey$$1]) {
        if (cookieMap.userProducts) {
          ...
          if (userType === userTypes$1.registered && cookieMap[ssoKey$$1].firstName === 'guest' && cookieMap[ssoKey$$1].lastName === 'guest') {
            userType = userTypes$1.guest;
          }
        }
      }
      return userType;
            ...
            for (var _iterator2 = adSlot.rules[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
              ...
              if (adSlotRule.avoid === adSlotId) {
                (function () {
                  ...
                  if (parentResolvedWith && adSlotRule.onsize.split(',').find(function (sizeString) {
                    // eslint-disable-line
...
var AdManager = function () {
  ...
  _createClass(AdManager, [{
    ...
    */
    ...
    key: 'printShouldSendRequestToDfp',
    value: function printShouldSendRequestToDfp(id) {
      var res = '';
      if (!this.adSlots.has(id)) {
        res = 'id not exist: ' + id;
      } else {
        var a = this.adSlots.get(id);
    ...
    key: 'testShouldSendRequestToDfp',
    value: function testShouldSendRequestToDfp(id) {
      // leave the old name for ios legacy
      var cookieMap = getCookieAsMap();
      cookieMap = JSON.stringify(cookieMap);
      cookieMap = cookieMap.replace(/,/g, '<br>');
      var res = '<div>' + cookieMap + '</div>';
      document.write(res);
      return res;
    }
  }, {
    ...
    value: function shouldAppPromotionElementPop() {
      return typeof window.appPromotionElement !== 'undefined' && typeof window.appPromotionElement.getShouldPop === 'function' ? window.appPromotionElement.getShouldPop() : false;
    }
    ...
    value: function initGoogleTargetingParams() {
      if (window.googletag && window.googletag.apiReady) {
        ...
        var cityNames = this.getCityNames();
        if (cityNames) {
          pubads.setTargeting('cityName', [].concat(_toConsumableArray(cityNames)));
        }
    ...
    key: 'getCityNames',
    value: function getCityNames() {
      var cityNames = [];
      document.querySelectorAll('[data-location-city-name]').forEach(function (element) {
