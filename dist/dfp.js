    ...
    */
    ...
    /*
      ...
      testShouldSendRequestToDfp(id) {
        ...
        let res = '';
        // 1
        res += this.conflictResolver.isBlocked(a.id) === !1 ? '1' : '0';
        // 2
        res += a.isWhitelisted() ? '1' : '0';
        // 3
        res += a.isBlacklisted() === !1 ? '1' : '0';
        // 4
        res += this.shouldDisplayAdAfterAdBlockRemoval(a) ? '1' : '0';
        // 5
        res += this.shouldDisplayAdMaavaronAfterPayWallBanner(a) ? '1' : '0';
        // 6
        res += this.doesBreakpointContainAd(a) ? '1' : '0';
        // 7
        res += this.haveValidCookieForSmartphoneapp() ? '1' : '0';
        // 8
        res += this.doesUserTypeMatchBannerTargeting(a) ? '1' : '0';
        // 9
        res += this.user.impressionManager.reachedQuota(a.id) === !1 ? '1' : '0';
        // res += "\n 123456789"
        let cookieMap = getCookieAsMap();
        cookieMap = JSON.stringify(cookieMap);
        cookieMap = cookieMap.replace(/,/g, '<br>');
        res = `<div><br>${res}<br>123456789<br>${cookieMap}<br></div>`;
